...
namespace modoff.Services
{
    public class NeighborhoodService
    {
        ...
        public bool SaveNeighbors(Viking viking, string neighborUid, int slot) {
            modoff.Model.Neighborhood? neighborhood = viking.Neighborhood;
        
            if (neighborhood == null) // if viking has no neighborhood yet, create a default one
                viking.Neighborhood = new modoff.Model.Neighborhood {
                    VikingId = viking.Id,
                    Slot0 = this.slot0,
                    Slot1 = this.slot1,
                    Slot2 = this.slot2,
        ...
        public NeighborData GetNeighbors(string userId) {
            modoff.Model.Viking? viking = ctx.Vikings.FirstOrDefault(e => e.Uid == new Guid(userId));
